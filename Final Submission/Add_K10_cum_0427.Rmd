...
alldata <- data.frame(dplyr::select(alldata,
                                    ...
                                    genhealth,Kessler,Kessler_Sum,GAD_Sum,Loneliness,Risk
                                    ))
```
...
If we didn't group K10 and treat it as a continuous variable, here is the result of linear mixed effect model.

```{r K10 model cumu sum}
m8 <- lmer(Kessler_Sum ~ PetOwner + Wave + if_shelter + PetOwner*if_shelter + (1|workerId), data = alldata)
...
Interpretation: The effect of 'PetOwner' is not significant from the output above.
...
```{r K10 model summary cumu sum}
summary_info <- createCI_linear(m8,'PetOwnerYes','PetOwnerYes:if_shelteryes')
rownames(summary_info) <- c('when the if_shelter is no', 'when the if_shelter is yes')
summary_info %>%
  round(3) %>%
  kable(escape = F, booktabs = T, linesep = "",
        caption = 'K-10: The estimate of PetOwner effect and its 95% confidence interval')%>%
  kable_styling()
```
...
Take the first row for example, when the if_shelter is 'no', the Loneliness score of those who have pets ((PetOwner = Yes) are 0.663 (0.136, 1.191) **lower** than those who don't have pets (PetOwner = No). 
